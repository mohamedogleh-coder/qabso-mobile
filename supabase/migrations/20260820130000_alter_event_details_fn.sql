-- Alters event_details_fn so a customer who paid at the desk can open their
-- own booking.
--
-- The customer branch matched paid_user only, which a walk-in never has: they
-- had no account when they paid, so the desk kept their number on the payment
-- as payer_phone. generate_booking_time_seq_fn already badges such a booking
-- as theirs, and tapping it landed on a booking this function would not
-- return. Now both functions ask the same question.
--
-- Everything else is carried over unchanged from 20260817150000.
...
BEGIN
    ...
    -- The number this customer keeps on their account. A booking they paid
    -- for at the desk carries the number instead of their id, because a
    -- walk-in has no account at the time they pay.
    SELECT phone_number
    INTO v_my_phone
    ...
    RETURN QUERY
        ...
        WHERE e.id = p_event_id
          ...
          -- through a booking this user paid on — either as the signed-in
          -- payer, or as the number they left at the desk. The manager branch
          -- matches the stadium and says nothing about who paid, so it lets
          -- through every booking on a stadium this user runs.
          AND (EXISTS (SELECT 1
                       FROM public.transactions t
                       WHERE t.event_id = e.id
                         AND (t.paid_user = auth.uid()
                           OR t.payer_phone = v_my_phone))
            OR EXISTS (SELECT 1
