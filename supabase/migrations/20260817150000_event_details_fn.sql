-- Fixes event_details_fn, which failed on its first call with
-- "column ledger.transactions does not exist".
--
-- The ledger lateral built its json but never named the column it produced,
-- so the column came out as "coalesce" and the outer select could not find
-- ledger.transactions. The money lateral above it names each of its columns,
-- which is why only this one broke. The only change here is the AS
-- transactions on that lateral.
--
-- Nothing warned about it at CREATE time because plpgsql only checks that the
-- statements in a body parse, not that every column in them resolves — so the
-- function was created and failed when it first ran.
