...
-- an input parameter, and this renames p_payment_status to p_event_status,
-- which is the name the body has always used. The argument types are spelled
-- out so the DROP can only ever match this function, and IF EXISTS keeps the
-- migration re-runnable on a database where it was never created.
