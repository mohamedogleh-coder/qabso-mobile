-- Finds the stadiums a customer can play at on a given day.
--
-- The customer searches with the number of players and a date, and may add a
-- start time, their own position, and how far they are willing to travel.
-- Every stadium comes back once, carrying the one field that fits the group
-- best.
--
...
--
-- The earlier migration made this function return its working hours too.
-- Postgres will not let a replace change the columns a function returns, so
...
CREATE OR REPLACE FUNCTION public.search_stadiums_fn(
    p_capacity smallint,
    p_date date,
...
-- The field pick runs once per open stadium and always reads the same three
-- things: the stadium, whether the field takes bookings, and its size. The
-- foreign key alone is not indexed — Postgres does not do it on its own — so
-- without this every search scans the whole fields table per stadium.
CREATE INDEX IF NOT EXISTS idx_fields_stadium_capacity
    ON public.fields (stadium_id, capacity)
    WHERE allow_booking;


COMMENT ON FUNCTION public.search_stadiums_fn IS
    'Stadiums open on a date with the smallest field that holds the group, '
        'one row each, optionally free at a time and within a radius in km.';
