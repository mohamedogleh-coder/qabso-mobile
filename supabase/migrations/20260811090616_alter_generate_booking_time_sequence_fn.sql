-- Alters generate_booking_time_seq_fn so a day the stadium does not work
-- raises instead of returning []. An empty array now means only "the day is
-- open but offers nothing", which the client can no longer confuse with a
-- closed stadium.
--
